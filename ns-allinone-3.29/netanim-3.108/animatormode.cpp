...
bool
AnimatorMode::loadTraceFile (QString traceFileName)
{
  if (m_showPropertiesButton->isChecked ())
    {
  ...
  if (traceFileName == "")
    {
      showPopup ("Invalid trace file name");
      return false;
    }
  ...
  bool loaded = parseXMLTraceFile (traceFileName);
  StatsMode::getInstance ()->systemReset ();
  QApplication::processEvents ();
  AnimatorScene::getInstance ()->setSceneInfoText ("Please select an XML trace file using the file load button on the top-left corner", false);
  return loaded;
  ...
  if (fileDialog.exec ())
    {
      ...
      if (traceFileName != "")
        {
          if (loadTraceFile (traceFileName))
            m_fileOpenButton->setEnabled (true);
        }
    }
