...
class BaselineWiringTests(unittest.TestCase):
    ...
    def test_progress_aware_guard_is_not_in_clean_baseline_branch(self):
        """The baseline branch is based on main, not the progress-aware prototype."""
        text = ROUTING_CC.read_text()
        self.assertNotIn("IsProgressingRelay", text)
        self.assertNotIn("ShouldUseTrustAwareFallback", text)
